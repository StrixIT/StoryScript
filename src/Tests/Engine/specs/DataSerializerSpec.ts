import { IDataSynchronizer } from "../../../Engine/Interfaces/services/dataSynchronizer";
...
describe("DataSerializer", () => {
    ...
    beforeAll(() => {
        const storageServiceMock = getStorageServiceMock();
        const serializer = new DataSerializer();
        const synchronizerMock = <IDataSynchronizer>{
            updateModifiedEntities: () => {},
            updateModifiedEntity: () => {}
        }
        const dataService = new DataService(storageServiceMock, serializer, synchronizerMock, TESTGAMEPREFIX);
        RunGame(dataService);
    ...
    it("should recreate locations from a JSON clone of the world and warn when the order of functions in an array changed", function() {
        ...
        expect(consoleOutput).toEqual([
            `Function with key: garden|actions|0|execute was found but the hash does not match the stored hash (old hash: 416420323, new hash: 1867314870)! Did you change the order of actions in an array and/or change the function content? If you changed the order, you need to reset the game world. If you changed only the content, you can ignore this warning.`,
            `Function with key: garden|actions|1|execute was found but the hash does not match the stored hash (old hash: 1867314870, new hash: 416420323)! Did you change the order of actions in an array and/or change the function content? If you changed the order, you need to reset the game world. If you changed only the content, you can ignore this warning.`
        ]);
