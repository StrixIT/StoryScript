...
import description from './leftCorridor.html?raw'
...
export function LeftCorridor() {
    return Location({
        ...
        enterEvents: [
            ['HiddenPit',
                (game: IGame) => {
                    var damage = Math.floor(Math.random() * 6 + 1) - game.activeCharacter.vlugheid;
                    game.activeCharacter.currentHitpoints -= Math.max(0, damage);
