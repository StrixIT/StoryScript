...
import { updateModifiedEntities, updateModifiedEntity } from './updateHelpers';
        ...
        try {
            ...
            if (jsonData) {
                ...
                if (Array.isArray(data) && data[0]?.type && data[0]?.id) {
                    ...
                    updateModifiedEntities(data, pristineEntities, Object.keys(pristineCollection).map(k => pristineCollection[k]));
                }
                else {
                    const result = this.restoreObjects(functionList, null, data);
                    updateModifiedEntity(result, result, pristineEntities);
                }
