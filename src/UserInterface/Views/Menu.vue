<template>
  ...
  <div class="modal-body">
    <div v-if="state === PlayState.Menu" id="mainmenu">
    ...
    <div v-if="state === 'ConfirmRestart'" id="confirmrestart">
      <h4>
    ...
    <div v-if="state === 'Save' || state === 'Load'">
      <div class="row">
      ...
      <div class="row">
        <div class="col-12">
          <p v-if="state === 'Save' && overwriteSelected()" class="alert alert-danger">
            {{ texts.format(texts.overwriteSaveGame, [selectedGame]) }}</p>
          <p v-if="state === 'Load' && selectedGame" class="alert alert-success">
            {{ texts.format(texts.loadSaveGame, [selectedGame]) }}</p>
          <button v-if="state === 'Save'" :disabled="!selectedGame" class="btn btn-primary save-button" type="button"
                  @click="saveGame()">{{ texts.save }}
          </button>
          <button v-if="state === 'Load'" :disabled="!selectedGame" class="btn btn-primary save-button" type="button"
    ...
    <router-link :to="Routes.Main">{{ texts.backToGame }}</router-link>
  </div>
</template>
...
const state = ref<string>(PlayState.Menu);
const selectedGame = ref<string>(null);
const saveKeys = ref<string[]>([]);

const restart = (): string => state.value = 'ConfirmRestart';
